...
# 🔹 Concepts listesini düz metin olarak al
def get_concepts(work):
    concepts = work.get("concepts", [])
    # display_name'leri al ve aralarına noktalı virgül koy
    concept_names = [c["display_name"] for c in concepts if c.get("display_name")]
...
def clean_records(records):
    ...
    for work in records:
        ...
        concept_text = get_concepts(work)  # ← Concepts eklendi

        cleaned_data.append({
            "title": work.get("display_name"),
            "abstract": abstract_text,
            "concepts": concept_text,               # ← Concepts alanı CSV’ye ekleniyor
